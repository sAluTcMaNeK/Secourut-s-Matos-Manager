...
try { $pdo->exec("ALTER TABLE materiels ADD COLUMN fournisseur TEXT DEFAULT ''"); } catch (PDOException $e) {}
try { $pdo->exec("ALTER TABLE materiels ADD COLUMN check_fonctionnel TINYINT(1) DEFAULT 0"); } catch (PDOException $e) {}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'delete_materiel') {
        ...
        try {
            $nom = $pdo->query("SELECT nom FROM materiels WHERE id = $id")->fetchColumn();
            $pdo->prepare("DELETE FROM materiels WHERE id = :id")->execute(['id' => $id]);
            $pdo->prepare("INSERT INTO historique_actions (nom_utilisateur, action, date_action) VALUES (?, ?, NOW())")
                ->execute([$_SESSION['username'], "A supprimé la référence : $nom"]);
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'add_materiel') {
        ...
        if (!empty($nom) && !empty($categorie_id)) {
            ...
            if ($stmt_check->fetchColumn() > 0) {
                ...
                try {
                    $pdo->prepare("INSERT INTO materiels (nom, categorie_id, seuil_alerte, fournisseur, check_fonctionnel) VALUES (:nom, :cat_id, :seuil, :fournisseur, :check)")
                        ->execute(['nom' => $nom, 'cat_id' => $categorie_id, 'seuil' => $seuil_alerte, 'fournisseur' => $fournisseur, 'check' => $check_fonctionnel]);
                    $pdo->prepare("INSERT INTO historique_actions (nom_utilisateur, action, date_action) VALUES (?, ?, NOW())")
                        ->execute([$_SESSION['username'], "A ajouté une nouvelle référence : $nom"]);
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'edit_materiel') {
        ...
        if ($id > 0 && !empty($nom) && !empty($categorie_id)) {
            ...
            if ($stmt_check->fetchColumn() > 0) {
                ...
                try {
                    $pdo->prepare("UPDATE materiels SET nom=:nom, categorie_id=:cat_id, seuil_alerte=:seuil, fournisseur=:fournisseur, check_fonctionnel=:check WHERE id=:id")
                        ->execute(['nom' => $nom, 'cat_id' => $categorie_id, 'seuil' => $seuil_alerte, 'fournisseur' => $fournisseur, 'check' => $check_fonctionnel, 'id' => $id]);
                    $pdo->prepare("INSERT INTO historique_actions (nom_utilisateur, action, date_action) VALUES (?, ?, NOW())")
                        ->execute([$_SESSION['username'], "A modifié la référence : $nom"]);
...
$materiels_bruts = $pdo->query("
    SELECT m.id, m.nom, m.seuil_alerte, m.fournisseur, m.check_fonctionnel, m.categorie_id,
           c.nom AS categorie_nom
    FROM materiels m JOIN categories c ON m.categorie_id = c.id
    ...
    <div class="catalogue-sidebar">
        ...
        <div id="bloc-ajout" class="white-box">
            ...
            <form action="materiel.php" method="POST">
                ...
                <div class="check-func-add-wrapper mb-20">
                    <input type="checkbox" name="check_fonctionnel" value="1" id="check_func_add">
                    <label for="check_func_add" class="form-label">Check Fonctionnel</label>
                </div>
        ...
        <div id="bloc-edition" class="form-box-orange">
            ...
            <form action="materiel.php" method="POST">
                ...
                <div class="mb-15">
                    ...
                    <input type="number" name="seuil_alerte" id="edit_seuil_alerte" min="0" class="input-field input-edit-focus">
                </div>
                <div class="check-func-edit-wrapper mb-20">
                    <input type="checkbox" name="check_fonctionnel" value="1" id="edit_check_fonctionnel">
                    <label for="edit_check_fonctionnel" class="form-label-edit">Remplacer le comptage par un "Check Fonctionnel"</label>
                </div>
                <div class="flex-row-sm">
                    <button type="submit" class="btn btn-warning btn-flex-2">Enregistrer</button>
                    <button type="button" onclick="fermerEdition()" class="btn btn-secondary btn-flex-1">Annuler</button>
                </div>
            </form>
        </div>
    ...
    <div class="white-box catalogue-main">
        <div class="search-header">
            ...
            <div class="search-container">
                ...
                <input type="text" id="searchCatalogue" class="search-input" placeholder="Chercher un matériel..." onkeyup="filtrerCatalogue()">
            </div>
        </div>
                            ...
                            <tr class="catalogue-row" data-nom="<?php echo htmlspecialchars(strtolower($mat['nom'])); ?>">
                                <td class="item-name">
                                    ...
                                    <?php if ($mat['check_fonctionnel'] == 1): ?>
                                        <div class="badge-check-fonctionnel">⚙️ Vérif. Fonctionnelle</div>
                                    <?php endif; ?>
                                </td>
                                ...
                                <td class="text-center">
                                    <span class="text-warning font-bold"><?php echo htmlspecialchars($mat['seuil_alerte']); ?></span>
                                </td>
                                <?php if ($peut_editer): ?>
                                <td class="text-center">
                                    <div class="flex-center" style="justify-content:center;">
                                        ...
                                        <form method="POST" action="materiel.php" class="mb-0" onsubmit="return confirm('ATTENTION ! Supprimer ?');">
                                            <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
